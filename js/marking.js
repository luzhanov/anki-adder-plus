...
function splitText(id, searchForClozeMarkup, text) {
	...
	if (arguments.length == 2) {
		splitText(id, searchForClozeMarkup, id.innerHTML.replace(/&nbsp;/g,' ').replace(/(\r|\t|\v|\f)/g," ").replace(/&amp;/g,'&').replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>'));
		$(id).remove();
	} else {
...
function insertText(id, text, type) {
	...
	if (typeof type == "number") { //A cloze
		e.addClass("c").attr("data-c", type).html(text).on({
			"mousedown" : function(){wordMouseDown(this);},
			"mouseup" : function(){wordMouseUp(this);},
			"mouseover" : function(){wordMouseOver(this);}
		});
	} else {
...
function updateLayout() {
	...
	/* word. */$(".w, .p, .c").next(".p").addClass("nopaddingStart");
	/* .word */$(".w, .c").prev(".p").addClass("nopaddingEnd");
	/* cWORD */$(".c").next(".w").addClass("nopaddingStart"); //These are
	/* WORDc */$(".c").prev(".w").addClass("nopaddingEnd"); //only possible
	/* cC */$(".c").next(".c").addClass("nopaddingStart"); //through text editing
	...
	$("#clozefield .s").first().remove(); $("#clozefield .s").last().remove(); //Temporary spaces to get correct repositioning in the beginning and at the end
}
...
function wordMouseUp(id) {
	if (markEnd) {
		...
		$(".c[data-c="+searchC+"]").each(function(){splitText(this, false);});
		updateScrollWidth();
	...
	if (markStart && markEnd && markStart!=markEnd  && (markingC!=0 || ctrl || shift || alt || $(markStart).attr("data-c"))) {
		...
		if (isEarlier(markStart, markEnd)) {
			...
			$(markStart).nextUntil(markEnd).each(function() {
				...
				{combinedWords += $(this).html().replace(/&nbsp;/g,' '); prevIsSpace=($(this).html()=="&nbsp;");}
				$(this).remove();
			});
	...
	$("#clozearea").replaceWith($(document.createElement("div")).attr({"id":"clozefield", "tabindex":"0"})
		...
		.on({'focus' : function(){
				...
				if (!$(".w:hover, .p:hover, .s:hover, .c:hover").length) {
					...
					setTimeout(function(){$("#clozearea").focus()},1);
				}
				mouseDown = true;
			}
		})
