...
$(document.body).on('click', 'tr[data-deck-name] [data-action="remove"]', (ev) => {
  let decksCount = DeckMng.getListOfDecks().length;
  if (decksCount === 1) {
    // user shouldn't remove last deck
    return;
  }
  let deckName = $(ev.target).closest('[data-deck-name]').attr('data-deck-name');
  if (DeckMng.isDeckEmpty(deckName) || confirm(chrome.i18n.getMessage("confirmRemovingNonEmptyDeck"))) {
    DeckMng.removeDeck(deckName);
    updateDecksList();
  }
});
...
function optionsLocalDecks() {
  ...
  const listHtml = decksList.map((deckName) => `
